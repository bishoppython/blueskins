...
dbc.ModalBody([
        ...
        dbc.Modal(
        [
            dbc.ModalHeader(dbc.ModalTitle("Click no botão para excluir.",style={'color':'white'}),className = 'modal-color'),
            dbc.ModalBody([
                dbc.Button("Excluir", id="delete-conf", n_clicks=0, color="danger", style={ 'display': 'flex',
                'justify-content': 'center',  'align-items': 'center'}),
                # Outras componentes do corpo do modal podem ser adicionadas aqui.
            ],className = 'modal-color d-flex justify-content-center align-items-center" '),
...
def toggle_modal(n1, n2, is_open, type_event):
...
@app.callback(
    ...
    Input('delete-conf', 'n_clicks'),

    State('modal-delet', 'is_open'),
    prevent_initial_call=True
)
def toggle_modal(n,delet_conf, is_open):
    ...
    if 'delete_event' in changed_id and  summed_value > 0:
        return not is_open 
    elif 'delete-conf' in button_id:
        return not is_open
    return  is_open
...
def update_lista_eventos(n_clicks, delete_clicks,delete_conf, data_conc,  titulo, local, dep_pref, arr_pref, descricao, type_event,prioridade, store_data):
    ...
    if 'submit-tarefa' in changed_id:
        if  not titulo or not local or not dep_pref or not arr_pref or not descricao or not type_event or not  prioridade:
            ...
            conn.close()
            return  notificacao,f
    elif 'delete_event' in changed_id:
        ctx = dash.callback_context
        button_id = ctx.triggered[0]['prop_id'].split('.')[0]
...
def update_store_data(is_open, n_clicks, n_intervals, current_data):
    ctx = dash.callback_context
    button_id = ctx.triggered[0]['prop_id'].split('.')[0]    
    changed_id = [p['prop_id'] for p in callback_context.triggered][0]
    ...
    if 'delete-conf' == button_id and n_clicks > 0 :
        updated_data = seacher_banco_sql()
        return updated_data, a
